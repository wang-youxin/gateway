...
name: gateway
description: |
  万能入口 —— 所有 Skill 的统一调度器。内置 task-quantify 任务量化引擎。
  维护一份持久化能力索引（index.json），装新 Skill 时自动刷新。
  每次请求：先深度分析需求 → 读索引匹配 → 拆解分派。
  任务拆解/量化类需求不外派，直接调用内置 task-quantify 子能力执行完整 7-Phase 量化流程。
...
Phase 1: 深度需求分析（不准看索引）
  ↓
Phase 2: 读 index.json 匹配
  ├── 任务量化类 → 内置 task-quantify 子能力（不外派）
  └── 其他领域 → 交叉评分匹配
  ↓
Phase 3: 需求 × 能力模块交叉匹配
  ↓
Phase 4: 复合任务拆解 + 分派（task-quantify 类直接执行）
  ↓
...
| 维度 | 要回答的问题 |
|------|-------------|
| **领域** | 商业诊断 / 内容创作 / 软件开发 / 任务拆解 / 学习 / 决策 / 设计 / 运维 / 其他 |
| **任务类型** | 分析诊断 / 生成创作 / 搜索查找 / 学习理解 / 决策选择 / 转换格式 / 搭建系统 / 任务量化 |
...
### 内置子能力：task-quantify（任务量化）

当 Phase 1 判定领域为「任务拆解」或任务类型为「任务量化」时，**不读索引、不外派**，直接进入 task-quantify 子能力执行。

**触发条件（满足任一即命中）：**
- 用户明确要求拆解任务、量化工作、生成任务清单
- 用户说"帮我拆解"、"量化一下"、"把这个变成可执行计划"
- 用户描述了一个目标但不知道具体怎么做、需要分步骤

**执行方式：**
Gateway 的 Phase 1 需求分析结果直接作为 task-quantify 的 Phase 1 输入。task-quantify 根据复杂度自动选择简化/完整模式，执行后续 Phase 并输出量化任务地图。用户感知为 Gateway 一站式完成，不知道背后调了 task-quantify。

**task-quantify 的 11 个子能力模块（见 index.json）：**
```
mode-router       → 自适应模式判定（简化/完整）
requirement-analysis → Phase 1 需求解析
user-journey      → Phase 2 用户旅程反推（完整模式）
product-design    → Phase 3 产品设计（完整模式）
research-correction → Phase 4 调研纠错（完整模式）
reuse-first       → Phase 5 复用优先开发（完整模式）
deployment-reverse → Phase 6 部署反推（完整模式）
optimization-delivery → Phase 7 优化与交付
task-compiler     → 量化任务地图编译器
self-check        → 11 项自检清单
review-loop       → 复盘闭环
```
